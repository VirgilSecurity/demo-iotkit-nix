...
static void *
prvWaitForStart(void *pvParams);
static void
prvSuspendSignalHandler(int sig);
static void
prvResumeSignalHandler(int sig);
static void
prvSetupSignalsAndSchedulerPolicy(void);
static void
prvSuspendThread(pthread_t xThreadId);
static void
prvResumeThread(pthread_t xThreadId);
static pthread_t
prvGetThreadHandle(xTaskHandle hTask);
static portLONG
prvGetFreeThreadState(void);
static void
prvSetTaskCriticalNesting(pthread_t xThreadId, unsigned portBASE_TYPE uxNesting);
static unsigned portBASE_TYPE
prvGetTaskCriticalNesting(pthread_t xThreadId);
static void
prvDeleteThread(void *xThreadId);
/*-----------------------------------------------------------*/
...
vPortYield(void);
void
vPortSystemTickHandler(int sig);
...
pxPortInitialiseStack(portSTACK_TYPE *pxTopOfStack, pdTASK_CODE pxCode, void *pvParameters) {
    ...
    if (0 == pthread_mutex_lock(&xSingleThreadMutex)) {
        xSentinel = 0;
        if (0 != pthread_create(&(pxThreads[lIndexOfLastAddedTask].hThread),
                                &xThreadAttributes,
                                prvWaitForStart,
...
void
vPortClearInterruptMask(portBASE_TYPE xMask) {
...
void
vPortForciblyEndThread(void *pxTaskToDelete) {
...
void
vPortAddTaskHandle(void *pxTaskHandle) {
    portLONG lIndex;
...
vAssertCalled(const char *pcFile, unsigned long ulLine) {
    printf("Asserted in file %s, line %lu", pcFile, ulLine);
