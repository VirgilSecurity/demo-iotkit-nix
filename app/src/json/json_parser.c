...
static bool
json_token_streq(char *js, jsontok_t *t, char *s) {
 ...
 */
static jsontok_t *
skip_to_last(jsontok_t *element) {
    jsontok_t *t = element;
    if (t->size == 0)
        return t;
    int cnt = t->size;
...
static int
json_str_to_bool(jobj_t *jobj, jsontok_t *t, bool *value) {
...
static int
json_str_to_int(jobj_t *jobj, jsontok_t *t, int *value) {
    if (!t || t->type != JSMN_PRIMITIVE)
        return -WM_E_JSON_INVALID_TYPE;
    char *endptr;
...
static int
json_str_to_int64(jobj_t *jobj, jsontok_t *t, int64_t *value) {
    if (!t || t->type != JSMN_PRIMITIVE)
        return -WM_E_JSON_INVALID_TYPE;
    char *endptr;
...
static int
json_str_to_float(jobj_t *jobj, jsontok_t *t, float *value) {
    if (!t || t->type != JSMN_PRIMITIVE)
        return -WM_E_JSON_INVALID_TYPE;
...
static int
json_str_to_str(jobj_t *jobj, jsontok_t *t, char *value, int maxlen) {
 ...
 */
static int
json_get_value(jobj_t *jobj, char *key, jsontok_t **val_t) {
 ...
 */
static int
json_get_array_index(jobj_t *jobj, uint16_t index, jsontok_t **val_t) {
    *val_t = NULL;
    if (jobj->cur->type != JSMN_ARRAY)
...
static void
json_obj_init(jobj_t *jobj, jsontok_t *tokens, int num_tokens) {
    jobj->js = NULL;
    jobj->tokens = tokens;
...
static bool
__json_is_valid(jobj_t *jobj, jsmnenumtype_t type) {
    jsontok_t *t = jobj->cur;
...
static bool
json_is_array_valid(jobj_t *jobj) {
    return __json_is_valid(jobj, JSMN_ARRAY);
}

static bool
json_is_object_valid(jobj_t *jobj) {
